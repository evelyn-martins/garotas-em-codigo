...
interface CreatePostModalProps {
    onClose: () => void;
    onPostCreated: (post: IPostFeedWithLikes) => void;
}
...
export default function CreatePostModal({ onClose, onPostCreated }: CreatePostModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        setError('');
        try {
            const post = await PostService.createPost(trimmed);
            onPostCreated(post);
        } catch {
        ...
        >
            ...
            >
                <div className="mb-4 flex items-center justify-between">
                    <h2 className="text-base font-bold text-color-logo">Nova publicação</h2>
                    <button
                        onClick={onClose}
